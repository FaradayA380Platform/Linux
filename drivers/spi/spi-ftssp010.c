...
#define FTSSP010_CR0_SPI_FLASH              (1 << 11)               /* SPI application is Flash */
...
#define FTSSP010_CR0_SPI_FLASHTX            (1 << 18)               /* Flash Transmit Control */

/*
 * Control Register 1
 */
#define FTSSP010_CR1_SCLKDIV(x)             (((x) & 0xffff) << 0)   /* SCLK divider */
#define FTSSP010_CR1_SDL_MASK               (0x7f << 16)            /* serial  data length */
...
#define FTSSP010_CR2_RXEN                   (1 << 7)                /* Enable Rx */
#define FTSSP010_CR2_TXEN                   (1 << 8)                /* Enable Tx */
#define FTSSP010_CR2_FS                     (1 << 9)                /* CS line: 0 is low, 1 is high */
#define FTSSP010_CR2_CHIPSEL(x)             ((x & 0x3) << 10)       /* Chip Select */
#define FTSSP010_CR2_CHIPSEL_MASK           (0x3 << 10)             /* Chip Select */
...
#define FTSSP010_STATUS_GET_RFVE(reg)       (((reg) >> 4) & 0x3f)   /* receive  FIFO valid entries */
#define FTSSP010_STATUS_GET_TFVE(reg)       (((reg) >> 12) & 0x3f)  /* transmit FIFO valid entries */
...
#define FTSSP010_ICR_TFTHOD(x)              (((x) & 0x1f) << 12)    /* transmit FIFO threshold */
#define FTSSP010_ICR_RFTHOD_UNIT            (1 << 17)               /* receive FIFO threshold unit */
#define FTSSP010_ICR_TXCIEN                 (1 << 18)               /* transmit complete interrupt */
...
#define FTSSP010_ISR_TXCI                   (1 << 5)                /* transmit data complete */
				   ...
				   int wsize)
{
	...
	while (len > 0) {
		...
		do {
			...
			if ((sts & FTSSP010_STATUS_BUSY) == 0) {
				if (tx_buf) {
					if (FTSSP010_STATUS_GET_TFVE(sts) == 0)
						break;
				} else {
					break;
				}
		...
		if (rx_buf) {
			...
			for (i = 0; i < size; ++i) {
				wait_event(ctrl->waitq,
				           ftssp010_rxfifo_valid_entries(ctrl->base));
...
static void ftssp010_spi_chipselect(struct spi_device *spi, bool is_high)
{
	...
	if (is_high) {
		//Pull up the CS line.
		cr2 = (FTSSP010_CR2_CHIPSEL(spi->chip_select) |
		       FTSSP010_CR2_FS |
		       FTSSP010_CR2_SSPEN);
	} else {
...
static void ftssp010_spi_master_setup_mode(struct spi_device *spi)
{
	...
	cr0 = FTSSP010_CR0_FFMT_SPI |
	      FTSSP010_CR0_FSPO | FTSSP010_CR0_OPM_MASTER_STEREO;
				   ...
				   struct spi_transfer *t)
{
	...
	cr1 &= ~FTSSP010_CR1_SCLKDIV(0xFFFF);
	cr1 |= FTSSP010_CR1_SCLKDIV(div);
				    ...
				    struct spi_transfer *transfer)
{
	...
	dev_dbg(&spi->dev, "xfer_one: len %d, tx_buf %p rx_buf %p\n",
...
static int ftssp010_spi_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "Faraday FTSSP010 Controller HW version 0x%08x\n",
	         readl(ctrl->base + FTSSP010_OFFSET_REVISION));
