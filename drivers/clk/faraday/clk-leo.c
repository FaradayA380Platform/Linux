...
struct leo_clk_pll_factor {
	struct clk_hw   hw;
	void __iomem    *mult_reg;
...
#define to_leo_clk_pll_factor(_hw) container_of(_hw, struct leo_clk_pll_factor, hw)
...
static unsigned long faraday_clk_composite_pll_recalc_rate(struct clk_hw *hw, unsigned long parent_rate)
{
	struct leo_clk_pll_factor *pll = to_leo_clk_pll_factor(hw);
	unsigned long mult, rate;

	mult = (readl(pll->mult_reg) >> pll->mult_shift) & ((1 << pll->mult_width) - 1);

	rate = (parent_rate * mult) / pll->div;

	return rate;
}

static const struct clk_ops faraday_clk_composite_pll_ops = {
	.recalc_rate = faraday_clk_composite_pll_recalc_rate,
};
...
static void __init of_leo_faraday_pll_setup(struct device_node *np)
{
	struct clk *clk;
	struct clk_mux *mux;
	struct leo_clk_pll_factor *pll;
	const char *clk_name = np->name;
	const char *parent_name;
	u32 reg, shift, width, div;
	...
	pll = kzalloc(sizeof(struct clk_fixed_factor), GFP_KERNEL);
	if (!pll)
		return;
	...
	if (of_property_read_u32(np, "mult-reg", &reg)) {
		pr_err("%s: clock <%s> must have a mult-reg property\n",
			__func__, np->name);
	}

	pll->mult_reg = ftscu100_base + reg;
	
	if (of_property_read_u32(np, "mult-shift", &shift)) {
		pr_err("%s: clock <%s> must have a mult-shift property\n",
			__func__, np->name);
	}

	pll->mult_shift = shift;
	
	if (of_property_read_u32(np, "mult-width", &width)) {
		pr_err("%s: clock <%s> must have a mult-width property\n",
			__func__, np->name);
	}

	pll->mult_width = width;
	...
	clk = clk_register_composite(NULL, clk_name,
					  &parent_name, 1,
					  &mux->hw, &clk_mux_ops,
					  &pll->hw, &faraday_clk_composite_pll_ops,
					  NULL, NULL,
...
static const __initconst struct of_device_id leo_clk_match[] = {
	...
	{ .compatible = "leo,pll1_div4", .data = of_fixed_factor_clk_setup, },
