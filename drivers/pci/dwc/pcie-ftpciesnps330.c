...
static int ftpciesnps330_pcie_host_init(struct pcie_port *pp)
{
	struct dw_pcie *pci = to_dw_pcie_from_pp(pp);
	struct ftpciesnps330_pcie *hw = to_ftpciesnps330_pcie(pci);
...
static int __init ftpciesnps330_pcie_probe(struct platform_device *pdev)
{
	...
	hw->phy_base = devm_ioremap_resource(&pdev->dev, phy_base);
	if (IS_ERR(hw->phy_base)) {
		ret = PTR_ERR(hw->phy_base);
		goto map_failed;
	}
