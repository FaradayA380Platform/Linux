...
#define CONFIG_FTNANDC024_HCLK				50000000
#endif

/* Reserved BI byte (bad block information) from spare location, BI_byte = 1~6 */
#define CONFIG_BI_BYTE		1
//#define CONFIG_BI_BYTE		0

/* Debug level 
#define CONFIG_FTNANDC024_DEBUG			1*/
...
struct ftnandc024_nand_data {
	...
	int (*write_oob) (struct mtd_info *mtd, uint8_t *buf, uint32_t len);
	int (*read_oob) (struct mtd_info *mtd, uint8_t *buf);
	int (*write_page) (struct mtd_info * mtd, const uint8_t * buf);
	int (*read_page) (struct mtd_info *mtd, uint8_t *buf);
	int (*set_param) (struct mtd_info *mtd);
	int (*terminate) (struct mtd_info *mtd);
...
extern int ftnandc024_nand_read_page(struct mtd_info *, struct nand_chip *,
			uint8_t *, int, int);
extern int ftnandc024_nand_write_page_lowlevel(struct mtd_info *,
...
extern int ftnandc024_nand_write_oob_std(struct mtd_info *,
			struct nand_chip *, int);
extern int ftnandc024_nand_read_page_lp(struct mtd_info *, uint8_t *);
extern int ftnandc024_nand_write_page_lp(struct mtd_info *, const uint8_t *);
extern int ftnandc024_nand_read_oob_lp(struct mtd_info *, uint8_t *);
extern int ftnandc024_nand_write_oob_lp(struct mtd_info *, uint8_t *, uint32_t);
extern int ftnandc024_nand_read_page_sp(struct mtd_info *, uint8_t *);
extern int ftnandc024_nand_write_page_sp(struct mtd_info *, const uint8_t *);
extern int ftnandc024_nand_read_oob_sp(struct mtd_info *, uint8_t *);
extern int ftnandc024_nand_write_oob_sp(struct mtd_info *, uint8_t *, uint32_t);
