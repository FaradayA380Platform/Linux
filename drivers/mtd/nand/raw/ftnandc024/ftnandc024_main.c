...
static struct ftnandc024_nandchip_attr nand_attr[] = {
	...
	{"ESMT F59L1G81MB",
		64, 4, 9, 3, 64, 1, LEGACY_FLASH},	/* 2K SLC */
	{"Micron MX30LF4G28AD ",
		256, 8, 9, 4, 64, 1, LEGACY_FLASH},	/* 4K SLC */
	{"Spansion S34ML01G100TF100",
           64, 4, 9, 1, 64, 1, LEGACY_FLASH},	/* 2K SLC */         
};
...
static void ftnandc024_set_warmup_cycle(struct mtd_info *mtd,
			u8 wr_cyc, u8 rd_cyc)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
...
static void ftnandc024_onfi_set_feature(struct mtd_info *mtd, int val)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
...
static uint32_t ftnandc024_onfi_get_feature(struct mtd_info *mtd, int type)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
	struct cmd_feature cmd_f;
...
static void ftnandc024_onfi_config_DDR2(struct mtd_info *mtd, u8 wr_cyc, u8 rd_cyc)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
	struct cmd_feature cmd_f;
	...
	for (i = 0; i< 4; i ++) {
		writeb(param[i], data->io_base + SPARE_SRAM +
	...
	// Set the controller
	ftnandc024_set_warmup_cycle(mtd, wr_cyc, rd_cyc);
}
...
static int ftnandc024_onfi_sync(struct mtd_info *mtd)
{
	...
	val = ftnandc024_onfi_get_feature(mtd, LEGACY_FLASH);
	...
	if (data->flash_type == ONFI2) {
		ftnandc024_onfi_set_feature(mtd, 0x11);

		val = ftnandc024_onfi_get_feature(mtd, ONFI2);
		...
		ftnandc024_onfi_config_DDR2(mtd, 0, 0);

		ftnandc024_onfi_set_feature(mtd, 0x21);

		val = ftnandc024_onfi_get_feature(mtd, ONFI3);
		printk("NV-DDR2 feature for Ch %d, CE %d: 0x%x\n", data->cur_chan, data->sel_chip, val);
		if (val != 0x2121) {
			// Reset the setting
			ftnandc024_set_warmup_cycle(mtd, 0, 0);
			goto out;
		}
	}
	ret = 0;
...
static void ftnandc024_read_raw_id(struct mtd_info *mtd)
{
	...
	if(data->flash_type == ONFI2 || data->flash_type == ONFI3) {
		ftnandc024_onfi_set_feature(mtd, 0x00);
	}
	...
	memcpy_fromio(data->flash_raw_id, data->io_base + SPARE_SRAM + (data->cur_chan << data->spare_ch_offset) , id_size);
#if 0
        for (i = 0; i< id_size; i++) {
                data->flash_raw_id[i] = readb(data->io_base + SPARE_SRAM + (data->cur_chan << data->spare_ch_offset) + i);
        }
#endif
...
void ftnandc024_calibrate_dqs_delay(struct mtd_info *mtd)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
	struct cmd_feature cmd_f;
	...
	for(i = 0; i <= max_dqs_delay; i++) {
		...
		if(data->flash_type == ONFI2 || data->flash_type == ONFI3) {
			memcpy_fromio(p, data->io_base + SPARE_SRAM + (data->cur_chan<< data->spare_ch_offset), id_size_ddr);
#if  0
		        for (i = 0; i< id_size_ddr; i++) {
		                *p++ = readb(data->io_base + SPARE_SRAM + (data->cur_chan << data->spare_ch_offset) + i);
		        }
#endif
			...
			memcpy_fromio(p, data->io_base + SPARE_SRAM + (data->cur_chan<< data->spare_ch_offset), id_size_ddr-1);
#if 0
		        for (i = 0; i< id_size_ddr-1; i++) {
		                *p++ = readb(data->io_base + SPARE_SRAM + (data->cur_chan << data->spare_ch_offset) + i);
		        }
#endif
...
static void ftnandc024_calibrate_rlat(struct mtd_info *mtd)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
	struct cmd_feature cmd_f;
	...
	for(i = 0; i <= max_rlat; i++) {
		...
		memcpy_fromio(p, data->io_base + SPARE_SRAM + (data->cur_chan<< data->spare_ch_offset), id_size);
#if 0
	        for (i = 0; i< id_size; i++) {
	                *p++ = readb(data->io_base + SPARE_SRAM + (data->cur_chan << data->spare_ch_offset) + i);
	        }
...
static void ftnandc024_t2_get_feature(struct mtd_info *mtd, u8 *buf)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
	struct cmd_feature cmd_f;
...
static void ftnandc024_t2_set_feature(struct mtd_info *mtd, u8 *buf)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
	struct cmd_feature cmd_f;
...
static int ftnandc024_t2_sync(struct mtd_info *mtd, u8 wr_cyc, u8 rd_cyc)
{
	u8 param[4];
	...
	ftnandc024_t2_set_feature(mtd, param);

	// Get feature
	ftnandc024_t2_get_feature(mtd, param);
...
void ftnandc024_nand_select_chip(struct mtd_info *mtd, int chipnr)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
	int chn = 0;
	...
	if (data->scan_state != 1) {
		while (chipnr != -1) {
			if (chipnr < data->valid_chip[chn]) {
				break;
			} else {
				chipnr = chipnr - data->valid_chip[chn];
				chn++;
			}
		}
		data->cur_chan = chn;
...
#if 1  //EVB specific, if not A369/A380 EVB with FPGA, maybe you should #if 0
	/* Setting the Ext-AHB in A369*/
	clk = clk_get(&pdev->dev, "nandc_clk");
	if (IS_ERR(clk))
...
*/
	...
	//lichun@20150526, let data->chip.buffers allocate by nand_scan_tail()
	...
	ftnandc024_set_default_timing(mtd);
	ftnandc024_set_warmup_cycle(mtd, 0, 0);
	...
	ftnandc024_read_raw_id(mtd);
	if(data->flash_type == TOGGLE1 || data->flash_type == TOGGLE2) {
		ftnandc024_calc_timing(mtd);
		ftnandc024_calibrate_dqs_delay(mtd);
	}
...
//~lichun
	...
	data->chip.ecc.write_page_raw = ftnandc024_nand_write_page_lowlevel;
	if (data->large_page) {
	...
	if(data->flash_type == LEGACY_FLASH) {
		ftnandc024_calc_timing(mtd);
		ftnandc024_calibrate_rlat(mtd);
	}
	else if(data->flash_type == TOGGLE2) {
#if defined(CONFIG_FTNANDC024_SAMSUNG_K9GCGY8S0A)
		ftnandc024_t2_sync(mtd, 0, 2);
#elif defined(CONFIG_FTNANDC024_TOSHIBA_TH58TEG7DCJBA4C) ||\
	defined(CONFIG_FTNANDC024_TOSHIBA_TH58TFT0DDLBA8H)
		ftnandc024_t2_sync(mtd, 3, 3);
#endif
	}
...
//	ret = mtd_device_register(mtd, NULL, NULL);
