 ...
 *
 * Copyright (C) 2019-2020 Faraday Technology Corp.
 */
#include <linux/io.h>
...
void ftnandc024_regdump(struct mtd_info *mtd)
{
...
void ftnandc024_abort(struct mtd_info *mtd)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
...
int ftnandc024_issue_cmd(struct mtd_info *mtd, struct cmd_feature *cmd_f)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
	int status;
...
void ftnandc024_set_default_timing(struct mtd_info *mtd)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
	int i;
...
int ftnandc024_nand_wait(struct mtd_info *mtd, struct nand_chip *chip)
{
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
...
void ftnandc024_fill_prog_code(struct mtd_info *mtd, int location,
		int cmd_index)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
...
void ftnandc024_fill_prog_flow(struct mtd_info *mtd, int *program_flow_buf,
		int buf_len)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
...
int byte_rd(struct mtd_info *mtd, int real_pg, int col, int len,
				uint8_t *spare_buf)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
	...
	for(i = 0; i < tmp_len; i += data->max_spare) {
		...
		ftnandc024_nand_wait(mtd, chip);
		memcpy_fromio(buf + i, data->io_base + SPARE_SRAM + (data->cur_chan << data->spare_ch_offset), cmd_len);
	...
	if(data->flash_type == TOGGLE1 || data->flash_type == TOGGLE2 ||
		data->flash_type == ONFI2 || data->flash_type == ONFI3) {
		if(col & 0x1)
			memcpy_fromio(spare_buf, buf + 1, len);
		else
			memcpy_fromio(spare_buf, buf, len);
	}
	else
		memcpy_fromio(spare_buf, buf, len);
...
int rd_pg_w_oob(struct mtd_info *mtd, int real_pg,
			uint8_t *data_buf, uint8_t *spare_buf)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
	struct cmd_feature cmd_f;
	...
	lbuf = (u32 *) data_buf;
#if 1
        for (i = 0; i < (mtd->writesize >> data->eccbasft); i ++) {
                // Read the sector sized data from BMC when every time the bmc region isn't empty
                while (readl(data->io_base + BMC_REGION_STATUS)  & BMC_REGION_EMPTY(0)) {
 //                               printk("sector %d, REGION_STATUS=0x%x\n", i, readl(data->io_base + BMC_REGION_STATUS));
                }


                if (data->eccbasft == 9) {  //sector size is 512 byte
                        for (j = 0; j < (512 >> 2); j ++)
                                *lbuf++ = *(volatile unsigned *)(data->chip.IO_ADDR_R);
                }
                else { //sector size is 1024 byte
                        for (j = 0; j < (1024 >> 2); j ++)
                                *lbuf++ = *(volatile unsigned *)(data->chip.IO_ADDR_R);
                }
        }
	...
	ftnandc024_nand_wait(mtd, chip);
	memcpy_fromio(spare_buf, data->io_base + SPARE_SRAM + (data->cur_chan << data->spare_ch_offset), mtd->oobsize); 

	return 0;
}

int rd_pg_w_oob_sp(struct mtd_info *mtd, int real_pg,
			uint8_t *data_buf, uint8_t *spare_buf)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
...
int rd_oob(struct mtd_info *mtd, int real_pg, uint8_t *spare_buf)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
	struct cmd_feature cmd_f;
...
static int rd_oob_sp(struct mtd_info *mtd, int real_pg, uint8_t *spare_buf)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
	struct cmd_feature cmd_f;
...
int rd_pg(struct mtd_info *mtd, int real_pg, uint8_t *data_buf)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
	struct cmd_feature cmd_f;
...
int rd_pg_sp(struct mtd_info *mtd, int real_pg, uint8_t *data_buf)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
	struct cmd_feature cmd_f;
...
int ftnandc024_check_bad_spare(struct mtd_info *mtd, int pg)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
...
int ftnandc024_nand_read_page(struct mtd_info *mtd, struct nand_chip *chip,
...
int ftnandc024_nand_write_page_lowlevel(struct mtd_info *mtd,
...
int ftnandc024_nand_read_oob_std(struct mtd_info *mtd, struct nand_chip *chip, int page)
{
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
...
int ftnandc024_nand_write_oob_std(struct mtd_info *mtd, struct nand_chip *chip, int page)
{
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
...
int ftnandc024_nand_read_page_lp(struct mtd_info *mtd, uint8_t *buf)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
...
int ftnandc024_nand_write_page_lp(struct mtd_info *mtd, const uint8_t * buf)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
	struct cmd_feature cmd_f;
	...
	if(w_wo_spare == 0) {
		memcpy_fromio((data->io_base + SPARE_SRAM + (data->cur_chan << data->spare_ch_offset)), p, mtd->oobsize);
	...
	lbuf = (u32 *) buf;
        ...
        for (i = 0; i < (mtd->writesize >> data->eccbasft); i ++) {
                ...
                while (readl(data->io_base + BMC_REGION_STATUS)  & BMC_REGION_FULL(0)) ;
...
int ftnandc024_nand_read_oob_lp(struct mtd_info *mtd, uint8_t *buf)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
...
int ftnandc024_nand_write_oob_lp(struct mtd_info *mtd, uint8_t *buf, uint32_t len)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
	struct cmd_feature cmd_f;
	...
	memcpy_fromio(data->io_base + SPARE_SRAM + (data->cur_chan << data->spare_ch_offset), buf, mtd->oobsize);
...
int ftnandc024_nand_read_page_sp(struct mtd_info *mtd, uint8_t *buf)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
	int status = 0;
...
int ftnandc024_nand_write_page_sp(struct mtd_info *mtd, const uint8_t * buf)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
	struct cmd_feature cmd_f;
	...
	if(w_wo_spare == 0) {
		memcpy_fromio((data->io_base + SPARE_SRAM + (data->cur_chan << data->spare_ch_offset)), chip->oob_poi, mtd->oobsize);
...
int ftnandc024_nand_read_oob_sp(struct mtd_info *mtd, uint8_t *buf)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
	int i, status = 0;
...
int ftnandc024_nand_write_oob_sp(struct mtd_info *mtd, uint8_t *buf, uint32_t len)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ftnandc024_nand_data *data = nand_get_controller_data(chip);
	struct cmd_feature cmd_f;
	...
	memcpy_fromio(data->io_base + SPARE_SRAM + (data->cur_chan << data->spare_ch_offset), buf, len);
