...
/* Cypress */
#define S26Hx512T	0x001A
#define S26Hx01GT	0x001B

/* SFDP Header */
#define SFDP_HEADER_1	0x4653
#define SFDP_HEADER_2	0x5044
#define SFDP_OFF_MFR	0x800
#define SFDP_OFF_ID	0x802
...
struct amd_flash_info {
	...
	const uint16_t write_buf_size;
	const uint8_t write_buf_timeout_typ;
	const uint8_t write_buf_timeout_max;
};
...
/* add DL */
		...
		.mfr_id 	= CFI_MFR_CYPRESS,
		.dev_id 	= S26Hx512T,
		.name		= "Cypress S26Hx512T",
		...
		.nr_regions	= 1,
		.regions	= {
			ERASEINFO(0x40000, 256)
		},
		.write_buf_size = 9,
		.write_buf_timeout_typ = 9,
		.write_buf_timeout_max = 2
	}, {
		.mfr_id 	= CFI_MFR_CYPRESS,
		.dev_id 	= S26Hx01GT,
		.name		= "Cypress S26Hx01GT",
		.devtypes	= CFI_DEVICETYPE_X16,
		.uaddr		= MTD_UADDR_0x0AAA_0x0555,
		.dev_size	= SIZE_128MiB,
		.cmd_set	= P_ID_AMD_STD,
		.nr_regions	= 1,
		.regions	= {
			ERASEINFO(0x40000, 512)
		},
		.write_buf_size = 9,
		.write_buf_timeout_typ = 9,
		.write_buf_timeout_max = 2
	...
	struct cfi_private *cfi)
{
	 ...
	 *
	 * Cypress modifications:
	 *  - skip SFDP table for Cypress devices with Hyperbus interface,
	 *  - added byte swap support (CONFIG_MTD_CFI_BE_BYTE_SWAP).
	 * The mask variable is still needed to isolate one device out of an
	 * interleaved setup, e.g. to isolate 1 byte from a 2x1 byte setup.
	 */
	do {
		...
		result.x[0] &= mask;
		if (map_word_equal(map, result, CMD(SFDP_HEADER_1))) {
			ofs = cfi_build_cmd_addr(SFDP_OFF_MFR + (bank << 8),
						map, cfi);
			result = map_read(map, base + ofs);
			result.x[0] &= mask;
		}
		bank++;
	}  while (map_word_equal(map, result, CMD(CFI_MFR_CONTINUATION)));
	...
	case 1:
		return result.x[0];
	case 2:
		return cfi16_to_cpu(map, result.x[0]);
	case 4:
		return cfi32_to_cpu(map, result.x[0]);
	}
}
	...
	struct cfi_private *cfi)
{
	...
	result = map_read(map, base + ofs);
	result.x[0] &= mask;
	if (map_word_equal(map, result, CMD(SFDP_HEADER_2))) {
		ofs = cfi_build_cmd_addr(SFDP_OFF_ID, map, cfi);
		result = map_read(map, base + ofs);
		result.x[0] &= mask;
	}
...
static int cfi_jedec_setup(struct map_info *map, struct cfi_private *cfi, int index)
{
	...
	cfi->cfiq->MaxBufWriteSize = jedec_table[index].write_buf_size;
	cfi->cfiq->BufWriteTimeoutTyp =
				jedec_table[index].write_buf_timeout_typ;
	cfi->cfiq->BufWriteTimeoutMax =
				jedec_table[index].write_buf_timeout_max;
